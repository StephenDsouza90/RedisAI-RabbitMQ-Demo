...
class Encode:
    ...
    @staticmethod
    def _encode_csv_data(x_cols: list, y_cols: list, transformed_data_path: str, model: str) -> Tuple[pd.DataFrame, OrdinalEncoder]:
        """
        ...
        """
        df = pd.read_csv(transformed_data_path, sep=';', encoding='utf-8', low_memory=False)

        # Rows with model to reduce size
    ...
    def run(self, x_cols: list, y_cols: list, transformed_data_path: str, encoded_data_path: str, ordinal_encoder_path: str, model: str):
        """
        Orchestrates the encoding process by calling the encode_csv_data,
        saving the encoded data, and saving the OrdinalEncoder.
        """
        df, oe = self._encode_csv_data(x_cols, y_cols, transformed_data_path, model)
